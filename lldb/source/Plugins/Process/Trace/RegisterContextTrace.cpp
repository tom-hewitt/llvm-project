...
RegisterContextTrace::RegisterContextTrace(Thread &thread,
...
RegisterContextTrace::~RegisterContextTrace() {
  delete m_reg_set0.registers;
  delete m_pc_reg_info.invalidate_regs;
  delete m_pc_reg_info.value_regs;
}

void RegisterContextTrace::InvalidateAllRegisters() {}

size_t RegisterContextTrace::GetRegisterCount() { return 1; }

const lldb_private::RegisterInfo *
RegisterContextTrace::GetRegisterInfoAtIndex(size_t reg) {
  if (reg)
    return nullptr;
  return &m_pc_reg_info;
}

size_t RegisterContextTrace::GetRegisterSetCount() { return 1; }

const lldb_private::RegisterSet *
RegisterContextTrace::GetRegisterSet(size_t reg_set) {
  if (reg_set)
    return nullptr;
  return &m_reg_set0;
}

bool RegisterContextTrace::ReadRegister(
...
bool RegisterContextTrace::WriteRegister(
    const lldb_private::RegisterInfo *reg_info,
  ...
  uint32_t reg_number = reg_info->kinds[eRegisterKindGeneric];
  if (reg_number == LLDB_REGNUM_GENERIC_PC) {
    m_pc_value = value.GetAsUInt64();
    return true;
  }
  return false;
}

bool RegisterContextTrace::ReadAllRegisterValues(
    lldb::WritableDataBufferSP &data_sp) {
  return false;
}

bool RegisterContextTrace::WriteAllRegisterValues(
    const lldb::DataBufferSP &data_sp) {
  return false;
}

uint32_t RegisterContextTrace::ConvertRegisterKindToRegisterNumber(
